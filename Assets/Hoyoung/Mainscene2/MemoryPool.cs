...
public class MemoryPool : IEnumerable, System.IDisposable
{
    ...
    // count : 풀 최고 갯수
    //-------------------------------------------------------------------------------------
    public void Create(Object original, int count, GameObject bullet_parent) // argument 하나 더 추가 ( 부모 오브젝트 아래에 풀 생성 시키기 위해)
    {
        Dispose();    // 메모리풀 초기화
        ...
        for (int i = 0; i < count; i++) // count 만큼 반복
        {
            ...
            table[i] = item;
            // 총알의 부모 는 player_fire 에서 정의된 부모오브젝트 (추가)
            item.gameObject.transform.parent = bullet_parent.transform;
        }
    }
