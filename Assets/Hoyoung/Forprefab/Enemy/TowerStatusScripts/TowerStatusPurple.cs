...
public class TowerStatusPurple : MonoBehaviour
{
    public int ArrayIndex;

    public float code = 5f;
    ...
    void System2()
    {
        // 2. 상극 
        // 목극토
        if (DamagingBullet.GetComponent<BulletCode>().code == 3)
        {

            damaged *= WeakenNum; // 10 * 1.1
            GameObject.Find("BuffManager").GetComponent<BuffManager>().DebuffEffect(this.transform);
        }
        else if (DamagingBullet.GetComponent<BulletCode>().code == 3.1)
        {
            damaged *= WeakenNum2; // 20 * 1.2 배
            GameObject.Find("BuffManager").GetComponent<BuffManager>().DebuffEffect(this.transform);
        }
        else if (DamagingBullet.GetComponent<BulletCode>().code == 3.2)
        {
