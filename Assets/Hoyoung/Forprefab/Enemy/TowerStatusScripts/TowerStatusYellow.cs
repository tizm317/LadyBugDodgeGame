...
public class TowerStatusYellow : MonoBehaviour
{
    public int ArrayIndex;

    public float code = 4f;


    public float TowerHP = 100.0f; // 타워의 현재 체력 
    public float MissileHP = 10.0f; // 노란타워의 유도미사일 HP
    ...
    void System1()
    {
        // 1. 상생 
        // 토생금
        if (DamagingBullet.GetComponent<BulletCode>().code == 5)
        {
            // 기본 데미지 입음
        ...
        else if (DamagingBullet.GetComponent<BulletCode>().code == 5.1)
        {
        ...
        else if (DamagingBullet.GetComponent<BulletCode>().code == 5.2)
        {
    ...
    void System2()
    {
        // 2. 상극 
        // 화극금
        if (DamagingBullet.GetComponent<BulletCode>().code == 1)
        {
            damaged *= WeakenNum; // 10 * 1.1
            GameObject.Find("BuffManager").GetComponent<BuffManager>().DebuffEffect(this.transform);
        }
        else if (DamagingBullet.GetComponent<BulletCode>().code == 1.1)
        {
            damaged *= WeakenNum2; // 20 * 1.2 배
            GameObject.Find("BuffManager").GetComponent<BuffManager>().DebuffEffect(this.transform);
        }
        else if (DamagingBullet.GetComponent<BulletCode>().code == 1.2)
        {
    ...
    void Update()
    {
        ...
        //float nextTowerHP = TowerHP + 10;
        //float nextMissileHP = MissileHP + 1;
        //float nextTowerAttackDamage = TowerAttackDamage + 0.2f;
        //int nextTowerLevel = TowerLevel + 1;

        //AliveTowerNumber = GameObject.Find("GameOverManager").GetComponent<GameOverScript>().TowerNumber; // 살아있는 타워 수는 타워 넘버에서 받아옴
